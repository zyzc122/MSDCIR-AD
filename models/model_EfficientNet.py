...
from GaussionBlur import get_gaussian_kernel
...
class CVAE(nn.Module):
	def __init__(self, n_dim1, n_dim2, n_embedding1, n_embedding2, size, seed=3407):
		...
		self.baselayer = self.backbone.features

		self.layer1 = self.baselayer[:5]
		self.layer2 = self.baselayer[5:8]

		self.layer_11 = nn.Sequential(nn.Conv2d(112, n_dim1, kernel_size=(3, 3), padding=1),
		                              nn.BatchNorm2d(n_dim1), nn.ReLU(inplace=True))

		self.layer_22 = nn.Sequential(nn.Conv2d(448, n_dim2, kernel_size=(3, 3), padding=1),
