...
class ExponentialLR(lr_scheduler._LRScheduler):
    def __init__(self, optimizer, lr_base=1e-3, decay=1e-3, warmup=200):
    ...
    def get_lr(self):
        if self.iteration < self.warmup:
            lr = (
                self.lr_base * (self.iteration + 1) / self.warmup
            )

        else:
            lr = self.lr_base * (1 - self.decay) ** (self.iteration - self.warmup + 1)
